...
Dashboard đọc **5 bảng** trong Lark Base (Bitable): `DS thu cũ`, `DS Tư vấn`,
`DS backup`, `Check in`, `Danh sách đơn hàng`. Có **2 cách** kết nối:
...
**3 bảng DS** (mỗi dòng = 1 bàn) cần các cột:
...
**Khối Status** (tên cột giống nhau ở cả 3 bảng DS):
...
Mã bàn (`STT bàn`) phải trùng `TC1..TC10 / TV1..TV18 / BK1..BK10` để khớp 38 node
trên sơ đồ (bàn nào không có trong data → hiện xám).
...
const TABLE = {                                    // table_id từng bảng
  dsTradein: process.env.TB_DS_TRADEIN,
  dsConsult: process.env.TB_DS_CONSULT,
  dsBackup:  process.env.TB_DS_BACKUP,
...
VITE_LARK_TABLE_DS_TRADEIN=<table_id DS thu cũ>
VITE_LARK_TABLE_DS_CONSULT=<table_id DS Tư vấn>
VITE_LARK_TABLE_DS_BACKUP=<table_id DS backup>
